...
public abstract class AbstractEventHandler<T extends CommonEvent> implements ApplicationListener<T>
{
	 ...
	 * @return
	 */
	public String parseModuleFromRequestUrl(String name)
	{
		// app下的模块,请求模式 /app/xx/..
		if (name.startsWith("/app/"))
		{

			return spliteAndGetModule("app", name);
			// sysapp下的模块,请求模式 /sysapp/xx/..
		}
		else if (name.startsWith("/sysapp/"))
		{
			return spliteAndGetModule("sysapp", name);

			// sysapp下的模块,请求模式 /baseapp/xx/..
		}
		else if (name.startsWith("/baseapp/"))
		{
			return spliteAndGetModule("baseapp", name);
