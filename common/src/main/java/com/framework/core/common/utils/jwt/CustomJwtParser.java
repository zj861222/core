...
package com.framework.core.common.utils.jwt;
...
/**
 * 自定义 JWTPARSER， 为了不抛出 ExpiredJwtException 这个异常
 * @author zhangjun
 *
 */
@SuppressWarnings("unchecked")
public class CustomJwtParser implements JwtParser {
	...
	public Jwt parse(String jwt) throws ExpiredJwtException, MalformedJwtException, SignatureException {
		...
		if (payload.charAt(0) == '{' && payload.charAt(payload.length() - 1) == '}') { // likely
																						// to
																						// be
																						// json,
																						// parse
		...
		// =============== Signature =================
		if (base64UrlEncodedDigest != null) { // it is signed - validate the
												// signature
			...
			if (key == null) { // fall back to keyBytes
				...
				if (Objects.isEmpty(keyBytes) && signingKeyResolver != null) { // use
																				// the
																				// signingKeyResolver
					if (claims != null) {
			...
			// re-create the jwt part without the signature. This is what needs
...
//			}
	...
	private void validateExpectedClaims(Header header, Claims claims) {
		for (String expectedClaimName : expectedClaims.keySet()) {
			...
			} else if (expectedClaimValue instanceof Date && actualClaimValue != null
			...
			if (actualClaimValue == null) {
				String msg = String.format(ClaimJwtException.MISSING_EXPECTED_CLAIM_MESSAGE_TEMPLATE, expectedClaimName,
						expectedClaimValue);
